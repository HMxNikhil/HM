...
export default function CodeBlockUI({
  code,
  language = "txt",
  filename,
  collapsible = true,
    ...
    <div className="rounded-lg overflow-hidden border border-white/10 bg-gray-900">
      <div className="flex items-center justify-between px-4 py-2 bg-black/50">
        ...
        <div className="flex items-center gap-2">
          {collapsible && (
            <button onClick={() => setOpen((s) => !s)} className="text-xs text-muted-foreground">
              {open ? "Collapse" : "Expand"}
            </button>
          )}
          <button onClick={copy} className="rounded px-2 py-1 bg-white/10 text-xs">
            Copy Code
          </button>
        </div>
      </div>
